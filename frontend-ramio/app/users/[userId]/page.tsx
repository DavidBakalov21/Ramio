...
function Avatar({ url, name, size = 88 }: { url: string | null; name: string | null; size?: number }) {
  if (url) {
    return (
      <Image
        src={url}
...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    api.get<PublicProfile>(`/user/${userId}`)
      .then((r) => {
        setProfile(r.data);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await api.patch<{ username: string | null; aboutMe: string | null; birthdate: string | null }>('/me', {
        username: username.trim() || undefined,
        aboutMe: aboutMe.trim() || undefined,
        birthdate: birthdate || undefined,
      });
      setProfile((prev) => prev ? { ...prev, username: res.data.username, aboutMe: res.data.aboutMe } : prev);
      setEditMode(false);
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await api.post<{ profilePictureUrl: string | null }>('/me/avatar', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      setProfile((prev) => prev ? { ...prev, profilePictureUrl: res.data.profilePictureUrl } : prev);
                  ...
                  <motion.form key="edit" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">JPEG, PNG, GIF or WebP · max 3 MB</p>
                      </div>
                    ...
                    <div>
                      ...
                      <input type="text" value={username} onChange={(e) => setUsername(e.target.value)}
                        maxLength={100} placeholder="Your display name"
                    ...
                    <div>
                      ...
                      <textarea value={aboutMe} onChange={(e) => setAboutMe(e.target.value)}
                        maxLength={5000} rows={4} placeholder="Tell others about yourself"
                        className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:border-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-200" />
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-3">
                        <h1 className="text-xl font-semibold text-slate-900 truncate">
                          {profile.username ?? <span className="text-slate-400 italic">No username</span>}
                        </h1>
                        {isOwnProfile && (
                      ...
                      {profile.aboutMe && (
                        <p className="mt-2 text-sm text-slate-600 whitespace-pre-wrap">{profile.aboutMe}</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                <div className="rounded-[1.6rem] border border-slate-200 bg-white p-6 shadow-sm">
                  ...
                  {profile.courses.length === 0 ? (
                    <p className="text-sm text-slate-500">
                      {profile.role === 'TEACHER' ? 'No courses created yet.' : 'Not enrolled in any courses yet.'}
                    </p>
                  ) : (
                      ...
                      {profile.courses.map((course) => (
                        <button key={course.id} type="button"
                          ...
                          {profile.role === 'STUDENT' && course.teacherName && (
                            <span className="text-xs text-slate-400">by {course.teacherName}</span>
                          )}
                        </button>
                      ))}
                    </div>
