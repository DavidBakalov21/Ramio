...
function deadlineLabel(deadline: string | null): { text: string; urgent: boolean } | null {
  ...
  if (diffH < 24) return { text: `Closes in ${Math.ceil(diffH)}h`, urgent: true };
    ...
    <section className="mb-8 flex w-full flex-col gap-3">
      ...
      {loading ? (
        ...
        <p className="text-sm text-slate-500">
          {isTeacher ? 'No quizzes yet. Use "Add" to create one.' : 'No quizzes yet.'}
        </p>
      ) : (
        <div className="flex flex-col gap-2">
          {quizzes.map((quiz) => {
            ...
            const isClosed = quiz.deadline && new Date(quiz.deadline) < new Date();
            ...
            if (!isTeacher) {
              if (!sub) {
                ...
                statusBadge = { label: 'In progress', color: 'bg-amber-100 text-amber-700' };
              } else {
                statusBadge = { label: 'Submitted', color: 'bg-emerald-100 text-emerald-700' };
              }
            }
            ...
            return (
              ...
              >
                <div className="flex flex-col gap-0.5 min-w-0">
                  <span className="text-sm font-medium text-slate-900 truncate">{quiz.title}</span>
                  <span className="text-xs text-slate-500">
                    {quiz.questionCount} question{quiz.questionCount !== 1 ? 's' : ''} · {quiz.totalPoints} pts
                    {quiz.timeLimit ? ` · ${quiz.timeLimit} min` : ''}
                  </span>
                  {dl && (
                    <span className={`text-xs font-medium ${dl.urgent ? 'text-red-600' : 'text-slate-500'}`}>
                      {dl.text}
                    </span>
                  )}
                </div>
                <div className="flex flex-col items-end gap-1 shrink-0">
                  ...
                  {!isTeacher && sub?.status === 'SUBMITTED' && sub.totalPoints != null && (
                    <span className="text-xs text-slate-500">
                      {Math.round(sub.totalPoints * 100) / 100} / {quiz.totalPoints} pts
                    </span>
                  )}
                </div>
              </button>
