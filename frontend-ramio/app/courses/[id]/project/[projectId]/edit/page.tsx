...
export default function EditProjectPage() {
  ...
  const loadProject = async () => {
    ...
    try {
      const [projectRes, submissionsRes] = await Promise.all([
        ...
        api.get<ProjectSubmissionListItem[]>(
          `/project/${projectId}/submissions?syncCodeBuild=1`,
        ),
      ]);
  ...
  const handleRunCodeBuild = async (submissionId: string) => {
    ...
    try {
      await api.post(
        `/project/${projectId}/submission/${submissionId}/codebuild-run`,
      );
    ...
    >
      <div className="mb-3 flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={() =>
            router.push(`/courses/${courseId}/project/${projectId}`)
          }
          ...
          {submissions.length === 0 ? (
            ...
            <ul className="space-y-2">
              {submissions.map((s) => {
                ...
                const p =
                  typeof s.codeBuildTestsPassed === 'number'
                    ? s.codeBuildTestsPassed
                    : 0;
                const f =
                  typeof s.codeBuildTestsFailed === 'number'
                    ? s.codeBuildTestsFailed
                    : 0;
                const sk =
                  typeof s.codeBuildTestsSkipped === 'number'
                    ? s.codeBuildTestsSkipped
                    : 0;
                const total = p + f + sk;
                const hasCounts =
                  typeof s.codeBuildTestsPassed === 'number' ||
                  ...
                  >
                    ...
                    <div className="flex gap-1">
                      <button
                        ...
                        className="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium text-slate-800 hover:bg-slate-50 disabled:opacity-60"
                      >
                        {codeBuildLoadingId === s.id
                          ? 'Starting…'
                          : s.codeBuildStatus === 'IN_PROGRESS'
                      ...
                      <button
                        ...
                        onClick={() =>
                          router.push(
                            `/courses/${courseId}/project/${projectId}/submission/${s.id}/assess`,
                          )
                        }
