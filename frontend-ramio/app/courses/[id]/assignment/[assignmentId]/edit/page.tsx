...
export default function EditAssignmentPage() {
  ...
  useEffect(() => {
    ...
    const isConfigured = configuredTests.some(
      (t) => t.language === activeTestTab,
    );
    ...
    const loadContent = async () => {
      try {
        const res = await api.get<string>(
          `/assignment/${assignmentId}/test-file/${activeTestTab}`,
        );
        setTestField(
          activeTestTab,
          'code',
  ...
  const handleSaveTest = async (lang: AssignmentLanguage) => {
    ...
    try {
      ...
      if (!fileToUpload) {
        showToast('No test code to save.', 'error');
        return;
      }
      ...
      showToast(
        `${ASSIGNMENT_LANGUAGE_MAP[lang].label} test saved.`,
        'success',
      );
    } catch {
  ...
  const handleDeleteTest = async (lang: AssignmentLanguage) => {
    if (
      !confirm(`Delete the ${ASSIGNMENT_LANGUAGE_MAP[lang].label} test file?`)
    )
      return;
    ...
    try {
      ...
      showToast(
        `${ASSIGNMENT_LANGUAGE_MAP[lang].label} test deleted.`,
        'success',
      );
    } catch {
  ...
  const handleGenerateTest = async (lang: AssignmentLanguage) => {
    ...
    try {
      const res = await api.post<{ code: string }>(
        `/assignment/${assignmentId}/test-file/${lang}/generate`,
      );
          ...
          <div className="border-b border-slate-200 px-3 pt-3">
            ...
            <div className="flex gap-1 flex-wrap">
              {ALL_LANGUAGES.map((lang) => {
                const configured = configuredTests.some(
                  (t) => t.language === lang,
                );
                return (
                  <button
              ...
              <div key={lang} className="p-3 space-y-2">
                {configured ? (
                  ...
                  <p className="text-xs text-slate-400 italic">
                    No test configured for {ASSIGNMENT_LANGUAGE_MAP[lang].label}{' '}
                    yet.
                  </p>
                )}
              ...
              {submissions.map((s) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {s.language ? (
                        <span className="mr-2 rounded bg-slate-100 px-1.5 py-0.5 font-mono">
                          {s.language}
                        </span>
                      ) : null}
                      {s.isChecked
                  ...
                  <button
                    ...
                    onClick={() =>
                      router.push(
                        `/courses/${courseId}/assignment/${assignmentId}/submission/${s.id}/assess`,
                      )
                    }
