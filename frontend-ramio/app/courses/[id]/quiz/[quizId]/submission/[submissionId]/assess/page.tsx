...
export default function AssessQuizSubmissionPage() {
  ...
  const [submission, setSubmission] = useState<QuizSubmissionDetail | null>(
    null,
  );
  ...
  const fetchSubmission = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get<QuizSubmissionDetail>(
      ...
      for (const q of res.data.questions) {
        if (q.type === 'OPEN_ANSWER' || q.type === 'CODING_TASK') {
          initial[q.id] = q.pointsEarned != null ? String(q.pointsEarned) : '';
        }
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.patch(`/quiz/submission/${submissionId}/assess`, {
        answers: answersToAssess,
      });
        ...
        <div className="mt-5 space-y-4">
          <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
            ...
            {submission.totalPoints != null && (
              <p className="mt-1 text-xs font-medium text-slate-700">
                Current score: {Math.round(submission.totalPoints * 100) / 100}{' '}
                / {submission.totalMax}
              </p>
            )}
          </div>
          ...
          {submission.questions.map((q, idx) => (
            ...
            >
              ...
              {q.imageUrl && (
                <QuizImage url={q.imageUrl} alt={`Question ${idx + 1} image`} />
              )}
                  ...
                  {q.answers.map((a) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {a.imageUrl && (
                          <QuizImage url={a.imageUrl} alt="Answer image" />
                        )}
                      </div>
                    );
                  })}
                  {q.pointsEarned != null && (
                    <p className="mt-1 text-xs text-slate-600">
                      ...
                      <span className="font-medium">
                        {Math.round(q.pointsEarned * 100) / 100} / {q.points}{' '}
                        pts
                      </span>
                    </p>
                  )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setOpenPoints((p) => ({ ...p, [q.id]: e.target.value }))
                      }
                ...
                <div className="space-y-3">
                  <p className="text-xs text-slate-500">
                    ...
                    {q.codingAutoPointsEarned != null && (
                      <span className="ml-2">
                        · Auto from tests:{' '}
                        {Math.round(q.codingAutoPointsEarned * 100) / 100} /{' '}
                        {q.points}
                      </span>
                    )}
                  </p>
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
                    ...
                    <pre className="max-h-48 overflow-auto whitespace-pre-wrap font-mono text-xs text-slate-800">
                      {q.openText || '(empty)'}
                    </pre>
                  </div>
