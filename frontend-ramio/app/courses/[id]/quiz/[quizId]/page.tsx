...
export default function QuizOverviewPage() {
  ...
  const loadSubmissions = useCallback(() => {
    ...
    api
      .get<QuizSubmissionSummary[]>(`/quiz/${quizId}/submissions`)
  ...
  const handleDeleteSubmission = async (submissionId: string) => {
    if (
      !confirm(
        "Reset this student's submission? They will be able to retake the quiz.",
      )
    )
      return;
  ...
  const deadlinePassed = quiz?.deadline
              ...
              <div className="mb-5 flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-3 text-xs text-slate-500">
                    ...
                    {quiz.deadline && (
                      <span
                        className={
                          deadlinePassed ? 'font-medium text-red-600' : ''
                        }
                      >
                        {deadlinePassed
                          ? '✕ Closed'
                ...
                {isTeacher && (
                  <button
                    type="button"
                    onClick={() =>
                      router.push(`/courses/${courseId}/quiz/${quizId}/edit`)
                    }
                        ...
                        <tbody>
                          {submissions.map((sub) => (
                            ...
                            >
                              ...
                              <td className="px-4 py-3">
                                <span
                                  className={`rounded-full px-2 py-0.5 text-xs font-medium ${sub.isFullyGraded ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}
                                >
                                  {sub.isFullyGraded
                                    ? 'Graded'
                                    : 'Pending review'}
                              ...
                              <td className="px-4 py-3 text-right">
                                <div className="flex items-center justify-end gap-3">
                                  <button
                                    ...
                                    onClick={() =>
                                      router.push(
                                        `/courses/${courseId}/quiz/${quizId}/submission/${sub.id}/assess`,
                                      )
                                    }
...
function StudentQuizActions({
  ...
  const [subStatus, setSubStatus] = useState<
    'loading' | 'none' | 'in_progress' | 'submitted'
  >('loading');
  const [subPoints, setSubPoints] = useState<number | null>(null);

  useEffect(() => {
    api
      .get<{ status: string; totalPoints: number | null }>(
        ...
        {subPoints != null && (
          <p className="mt-1 text-sm text-slate-600">
            ...
            <span className="font-semibold">
              {Math.round(subPoints * 100) / 100}
            </span>{' '}
            / {totalPoints} pts
            <span className="ml-1 text-xs text-slate-500">
              (open answers / manual coding tasks excluded until graded)
            </span>
          </p>
        )}
        <div className="mt-3">
          {quiz.allowReview ? (
            <button
              type="button"
              onClick={() =>
                router.push(`/courses/${courseId}/quiz/${quizId}/review`)
              }
