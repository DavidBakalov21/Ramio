...
export default function QuizReviewPage() {
  ...
  useEffect(() => {
    ...
    api
      .get<OwnQuizSubmission>(`/quiz/${quizId}/submission`)
      .then((r) => {
        if (!r.data.allowReview) setNotAvailable(true);
        else setSubmission(r.data);
      })
          ...
          {notAvailable ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
              ...
              <p className="mt-1 text-sm text-slate-500">
                The teacher has disabled review for this quiz.
              </p>
            </div>
          ) : !submission ? (
                ...
                <div className="mt-2 flex flex-wrap gap-3 text-sm text-slate-600">
                  ...
                  {submission.totalPoints != null && (
                    <span className="font-medium text-slate-900">
                      Score: {Math.round(submission.totalPoints * 100) / 100}{' '}
                      pts
                      <span className="ml-1 text-xs font-normal text-slate-500">
                        (open answers / manual coding tasks excluded until
                        graded)
                      </span>
                    </span>
                  )}
              ...
              <div className="space-y-4">
                {submission.questions.map((q, idx) => {
                  ...
                  const selectedIds = new Set(
                    q.answers.filter((a) => a.isSelected).map((a) => a.id),
                  );
                  ...
                  const codingMode =
                    ...
                    ('MANUAL_ONLY' as QuizCodingGradingMode);
                  const codingAwaitingTeacher =
                    q.type === 'CODING_TASK' &&
                    codingMode !== 'TESTS_ONLY' &&
                    q.pointsEarned == null;

                  let verdict: 'correct' | 'partial' | 'wrong' | 'open' | null =
                    null;
                  if (q.type === 'OPEN_ANSWER' || codingAwaitingTeacher)
                    verdict = 'open';
                        ...
                        <div className="flex shrink-0 flex-col items-end gap-0.5">
                          {showPoints &&
                            ...
                            q.pointsEarned != null && (
                              ...
                              >
                                {Math.round(q.pointsEarned * 100) / 100} /{' '}
                                {q.points} pts
                              </span>
                            )}
                          {showPoints &&
                            (q.type === 'OPEN_ANSWER' ||
                              q.type === 'CODING_TASK') && (
                              <span
                                className={`rounded-full px-2 py-0.5 text-xs font-medium ${q.pointsEarned != null ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}
                              >
                                {q.pointsEarned != null
                          ...
                          {q.answers.map((ans) => {
                            ...
                            if (showCorrect) {
                              if (isCorrect && isSelected)
                                bg = 'border-emerald-400 bg-emerald-50';
                              else if (isCorrect)
                                bg = 'border-emerald-300 bg-emerald-50/60';
                              else if (isSelected)
                            ...
                            return (
                              <div
                                key={ans.id}
                                className={`flex flex-col gap-1 rounded-xl border px-3 py-2.5 ${bg}`}
                              >
                      ...
                      {q.type === 'CODING_TASK' && (
                        <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
                          <div>
                            ...
                            {q.openText?.trim() ? (
                              <pre className="max-h-40 overflow-auto whitespace-pre-wrap font-mono text-xs text-slate-800">
                                {q.openText}
                              </pre>
                            ) : (
                          ...
                          {(q.codingTestStdout || q.codingTestStderr) && (
                            <div>
                              ...
                              <pre className="max-h-32 overflow-auto whitespace-pre-wrap font-mono text-[11px] text-slate-700">
                                {q.codingTestStderr?.trim() ||
                                  q.codingTestStdout}
                              </pre>
                            </div>
                          )}
