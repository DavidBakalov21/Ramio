...
export default function EditQuizPage() {
  ...
  useEffect(() => {
    ...
    api
      .get<Quiz>(`/quiz/${quizId}`)
      .then((r) => {
        setDeadline(
          r.data.deadline
            ? new Date(r.data.deadline).toISOString().slice(0, 16)
            : '',
        );
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-xs text-slate-500">
        You can change the time limit, deadline, question points, and which
        answers are correct. Changes to points or correct answers will
        recalculate all existing student scores.
      </p>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-600">
              Time limit in minutes (leave blank to remove)
            </label>
            <input
              type="number"
              min={1}
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-slate-600">
              Deadline (leave blank to remove)
            </label>
            <input
              type="datetime-local"
              value={deadline}
              ...
              {!isQuizOpenStyleQuestion(q.type as QuizQuestionType) ? (
                <div className="flex flex-col gap-2">
                  ...
                  {q.answers.map((a, ai) => (
                    <div
                      key={a.id}
                      className={`flex flex-col gap-1.5 rounded-xl border px-3 py-2.5 transition ${a.isCorrect ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200'}`}
                    >
